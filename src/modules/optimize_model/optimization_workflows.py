...
def optimize_model(
    ...
    model_input : str or cobra.Model
        Path to a multi-species model in any COBRApy supported format or a COBRApy model loaded into memory.
    diet_path : str
    ...
    return_outputs : bool, optional
        If True, will return the maximized UFEt fluxes, minimized IEX fluxes, and the model reaction bounds after minimization of the IEX fluxes, by default False.
    ...
    dict
        A dictionary containing the model reaction bounds after minimization of the IEX fluxes.
...
def optimize_model_mbx(
    ...
    diet_path: str,
    mbx_path: str,
    mbx_matched_key_path: str or dict,
    output_dir: str,
    verbose: bool = False,
    parallel: bool = False,
    n_jobs: int = 1,
    ...
    if os.path.exists(mbx_matched_key_path):
        with open(mbx_matched_key_path, "r") as f:
            mbx_matched_keys_dict = {
                line.split(":\t")[0]: line.split(":\t")[1].strip("\n") for line in f
            }
    else:
        mbx_output_path = os.path.join(output_dir, "mbx_bigg_ids.txt")
        mbx_matched_keys_dict = match_names_to_vmh(
            mbx_filepath=mbx_path, output_filepath=mbx_output_path, reuturn_ids=True
        )
    ...
    with open(f"{output_path}/{model.name}_min_mbx_opt_flux.txt", "w") as f:
        for rxn_id, value in min_opt_solutions.items():
