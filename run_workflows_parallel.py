...
for model_file in model_files:
    ...
    args_optimize_model = [f"{model_path}/{model_file}", output_path, False, True, False, True]
    args_optimize_model_mbx = [f"{model_path}/{model_file}", mbx_path, output_path, True, False, False, True]

    # Initialize ProcessPoolExecutor
    with concurrent.futures.ProcessPoolExecutor() as executor:
        # Submit the functions to the pool of workers
        future_optimize_model = executor.submit(optimize_model, *args_optimize_model)
